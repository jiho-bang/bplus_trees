...
class LeafNode extends BPlusNode {
    ...
    public Optional<Pair<DataBox, Long>> put(DataBox key, RecordId rid) {
        ...
        if (keys.contains(key)) {
            throw new BPlusTreeException("Duplicate");
        }
        ...
        if (max == keys.size()) {
            ...
            List<DataBox> rKeys = keys.subList(d, max);
            List<RecordId> lRids = rids.subList(0, d);
            List<RecordId> rRids = rids.subList(d, max);

            BPlusNode rLeaf = new LeafNode(metadata, bufferManager, rKeys, rRids, rightSibling, treeContext);
            this.keys = lKeys;
            this.rids = lRids;
            this.rightSibling = Optional.of(rLeaf.getPage().getPageNum());

            sync();
            return Optional.of(new Pair<>(rKeys.get(0), rLeaf.getPage().getPageNum()));
        }
        sync();
    ...
    public Optional<Pair<DataBox, Long>> bulkLoad(Iterator<Pair<DataBox, RecordId>> data,
        ...
        while (data.hasNext()) {
            Pair<DataBox, RecordId> nData = data.next();
            keys.add(nData.getFirst());
            rids.add(nData.getSecond());

            if (f + 1 == keys.size()) {
                List<DataBox> lKeys = keys.subList(0, f);
                List<DataBox> rKeys = keys.subList(f, f + 1);
                List<RecordId> lRids = rids.subList(0, f);
                List<RecordId> rRids = rids.subList(f, f + 1);

                BPlusNode nLeaf = new LeafNode(metadata, bufferManager, rKeys, rRids, rightSibling, treeContext);
                Long page = nLeaf.getPage().getPageNum();

                keys = lKeys;
                rids = lRids;
                rightSibling = Optional.of(page);

                sync();
                return Optional.of(new Pair<>(rKeys.get(0), nLeaf.getPage().getPageNum()));
            }
        }
        sync();
        return Optional.empty();
    ...
    public static LeafNode fromBytes(BPlusTreeMetadata metadata, BufferManager bufferManager,
        ...
        Long x = buf.getLong();
        Optional<Long> rightSibling = (x == -1) ? Optional.empty() : Optional.of(x);
