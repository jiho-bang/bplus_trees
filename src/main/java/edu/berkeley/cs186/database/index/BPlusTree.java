...
public class BPlusTree {
    ...
    public void put(DataBox key, RecordId rid) {
        ...
        if (pair.isPresent()) {
            ...
            InnerNode nRoot = new InnerNode(metadata, bufferManager, keys, children, lockContext);
            updateRoot(nRoot);
        }
        return;
    }
    ...
    public void bulkLoad(Iterator<Pair<DataBox, RecordId>> data, float fillFactor) {
        ...
        // Use the provided updateRoot() helper method to change
        // the tree's root if the old root splits.
        assert(root == root.getLeftmostLeaf());

        while (data.hasNext()) {
            Optional<Pair<DataBox, Long>> node = root.bulkLoad(data, fillFactor);
            if (node.isPresent()) {
                Long lChild = root.getPage().getPageNum();
                Long rChild = node.get().getSecond();
    ...
    private class BPlusTreeIterator implements Iterator<RecordId> {
        ...
        public boolean hasNext() {
            // TODO(proj2): implement
            if (iter.hasNext() || leaf.getRightSibling().isPresent()) {
                return true;
            }
            return false;
        }
