...
class InnerNode extends BPlusNode {
    ...
    public Optional<Pair<DataBox, Long>> put(DataBox key, RecordId rid) {
        ...
        keys.add(ind, pair.get().getFirst());
        children.add(ind + 1, pair.get().getSecond());
        ...
        List<DataBox> iKeys = new ArrayList<>();
        List<Long> iChildren = new ArrayList<>();
        for (int i = 0; i < d + 1; i++) {
            iKeys.add(keys.remove(d));
            iChildren.add(children.remove(d + 1));
        }

        DataBox nKey = iKeys.remove(0);
        InnerNode nNode = new InnerNode(metadata, bufferManager, iKeys, iChildren, treeContext);

        sync();
        return Optional.of(new Pair<>(nKey, nNode.getPage().getPageNum()));
    }
    ...
    public Optional<Pair<DataBox, Long>> bulkLoad(Iterator<Pair<DataBox, RecordId>> data,
        ...
        Optional<Pair<DataBox, Long>> mid = child.bulkLoad(data, fillFactor);

        if (mid.isPresent()) {
            int i = InnerNode.numLessThanEqual(mid.get().getFirst(), keys);
            keys.add(i, mid.get().getFirst());
            children.add(i + 1, mid.get().getSecond());

            if (keys.size() == (2 * d + 1)) {
                List<DataBox> lKeys = keys.subList(0, d);
                List<DataBox> rKeys = keys.subList(d + 1, 2 * d + 1);
                List<Long> lChildren = children.subList(0, d + 1);
                List<Long> rChildren = children.subList(d + 1, 2 * d + 2);

                DataBox nKey = keys.get(d);
                BPlusNode rNode = new InnerNode(metadata, bufferManager, rKeys, rChildren, treeContext);
                keys = lKeys;
                children = lChildren;

                sync();
                return Optional.of(new Pair<DataBox, Long>(nKey, rNode.getPage().getPageNum()));
            }
        }
